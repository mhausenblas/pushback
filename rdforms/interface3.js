...
function startHandleScan() {
	...
	$.get(	"relay.php", 
			{ URI : ref },
			function(rdfxml) {
				addStatus('loading RDF document done.');
				$("#progressbar").progressbar('value', 50);
				
				$('Description', rdfxml).each(function(index){
					//addStatus($(this).val());
					addStatus(index);
					numofLinks++;
				});			
				addStatus('scanning RDF document done.');
				if(debug) addStatus('<pre>' + htmlentities(rdfxml) + '</pre>');
				addStatus('Found ' + numofLinks + ' resources in the document.');
