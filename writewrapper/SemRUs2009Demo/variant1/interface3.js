...
function pushback() {
	...
	$.ajax({
	   type: "POST",
	   url: "http://localhost/writewrapper/SemRUs/version1/mapping.php",
...
function initSPARQLquery(rdf, randomnumber, debug) {
	...
	.where('?crudop	rdf:type ?crudoptype')
	.each(function () {
		if (this.crudoptype.value == "http://ld2sd.deri.org/pb/ns#CRUDOperationCREATE") { 
			SPARQLquery = SPARQLquery + "INSERT INTO <webid> \n";
		}
		else if (this.crudoptype.value == "http://ld2sd.deri.org/pb/ns#CRUDOperationDELETE") { 
			SPARQLquery = SPARQLquery + "DELETE FROM <webid> \n";
		}
